...
#' @param gunzip a logical value indicating whether or not files should only be renamed
#' (\code{gunzip = FALSE}) or renamed AND unzipped (\code{gunzip = TRUE}, default). Original
#' file will be removed.
#' @param gunzip_overwrite a logical value indicating to allow overwriting existing
#' uncompressed files (\code{gunzip_overwrite = TRUE}, default) or
#' abort if it exists (\code{gunzip_overwrite = FALSE}). Ignored if argument 'gunzip' is FALSE.
...
#' \item 2) Automatically rename files from e.g. \code{Saccharomyces_cerevisiae_cds_from_genomic_refseq.fna.gz} to \code{SaccharomycesCerevisiae.fna}.
...
#' }
#' @export
clean.retrieval <- function(x, gunzip = TRUE, gunzip_overwrite = TRUE) {
  stopifnot(is.logical(gunzip) & is.logical(gunzip_overwrite))
  ...
  folder_files <- list.files(dirname(x)[1], pattern = "\\.gz$", include.dirs = FALSE)

  if (length(folder_files) == 0)
    stop("Unfortunately, your specified folder '", x, "' does not include any compressed files.", call. = FALSE)
  ...
  } else folder_files_reduced <- folder_files
  ...
  if (gunzip) {
    for (i in seq_len(length(input_files))) {
      if (file.exists(file.path(dirname(x)[1], input_files[i]))) {
        ...
        R.utils::gunzip(file.path(dirname(x)[1], input_files[i]),
                        destname = file.path(dirname(x)[1], output_files[i]),
                        overwrite = gunzip_overwrite)
      }
    }
  }
